...
const RechargeRequestedSellCard = ({ requestedRechargeSellType }: any) => {
  ...
  if (requestedRechargeSellType.length <= 0) {
    return (
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="max-w-full overflow-x-auto">
        <table className="w-full table-auto">
          ...
          <tbody>
            {requestedRechargeSellType.map((item: any, key: number) => (
              <tr key={key}>
                <td className="border-b border-[#eee] flex items-center gap-4 py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                  <h5 className="font-medium text-black flex gap-3  dark:text-white">
                    {item.YohoId}
                  </h5>
                  <p>
                    <CopyToClipboard onCopy={onCopy} text={item.YohoId}>
                      <FaCopy className="text-blue-600 cursor-pointer" />
                    </CopyToClipboard>
                  </p>
                </td>
...
export default RechargeRequestedSellCard;
